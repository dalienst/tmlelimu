...
export default function SOPCertificatesPage() {
  ...
  return (
    <div className="p-6 md:p-8 w-full space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-700">
      {/* Header */}
      <div className="space-y-2">
        <div className="flex items-center gap-2 mb-1">
          <Badge className="bg-amber-50 text-amber-700 border border-amber-100 font-semibold uppercase tracking-wider text-[10px] px-2 py-0.5 rounded">
            Credentials
          </Badge>
        ...
        <h1 className="text-xl font-semibold tracking-tight text-[#004d40]">My Certificates</h1>
        <p className="text-sm text-zinc-500 font-medium max-w-2xl mt-1">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
        ...
        <div className="bg-white border border-zinc-150 rounded p-4 shadow-sm hover:shadow-md hover:border-amber-100 transition-all duration-300 flex items-center gap-3.5 group">
          ...
          <div>
            ...
            <h3 className="text-lg font-semibold text-zinc-800 mt-1">{certList.length} Earned</h3>
          </div>
        </div>
        ...
        <div className="bg-white border border-zinc-150 rounded p-4 shadow-sm hover:shadow-md hover:border-emerald-100 transition-all duration-300 flex items-center gap-3.5 group">
          ...
          <div>
            ...
            <h3 className="text-lg font-semibold text-zinc-800 mt-1">{latestCertDate}</h3>
          </div>
        </div>
      </div>
      ...
      {certList.length > 0 ? (
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
          {certList.map((cert) => (
            <Card key={cert.id} className="border border-zinc-150 shadow-sm hover:shadow-md transition-all duration-300 flex flex-col justify-between overflow-hidden bg-white group hover:border-amber-100 rounded">
              <div className="p-5 flex-1 flex flex-col gap-4">
                ...
                <div className="space-y-1">
                  <h3 className="text-sm font-semibold text-zinc-900 line-clamp-1 group-hover:text-[#004d40] transition-colors" title={cert.sop.title}>
                    {cert.sop.title}
                  </h3>
              ...
              <div className="px-5 pb-5 pt-0">
                <button
                  onClick={() => handleDownloadCertificate(cert.reference)}
                  disabled={downloadingCert === cert.reference}
                  className="w-full h-10 bg-[#004d40] hover:bg-[#00332b] text-white rounded text-xs font-semibold transition-all flex items-center justify-center gap-1.5 shadow-md shadow-emerald-900/10 cursor-pointer disabled:opacity-50"
                >
                  {downloadingCert === cert.reference ? (
        ...
        <div className="bg-zinc-50/50 border-2 border-dashed border-zinc-200 rounded p-16 text-center space-y-4 max-w-3xl mx-auto">
          <div className="w-16 h-16 bg-white rounded shadow-sm border border-zinc-100 flex items-center justify-center text-zinc-400 mx-auto">
            <Award className="w-8 h-8" />
          </div>
          <div className="space-y-1">
            <h3 className="text-sm font-semibold text-zinc-900">No Certificates Earned Yet</h3>
